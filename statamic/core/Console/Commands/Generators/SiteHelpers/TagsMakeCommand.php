...
class TagsMakeCommand extends AbstractCommand
{
    /**
     ...
     * @var string
     */
    protected $signature = 'make:tags-helper { --create-only : Only attempt to create the file, and do nothing if it exists. }';
     ...
     * @var string
     */
    protected $description = 'Generate a site helper tags file.';
    ...
    public function __construct()
    {
        ...
        $this->destination = site_path('helpers/Tags.php');
    }
    ...
    {
        File::copy(__DIR__.'/stubs/tags.stub', $this->destination);

        $this->info('Tags helper file created in ' . Path::makeRelative($this->destination));
    }
    ...
    private function update()
    {
        ...
        if (! $name = $this->ask('Enter the name of a tag to append to the file', false)) {
            return;
        }
    ...
    private function methodStub($name)
    {
        ...
        return str_replace(
            ...
            [$name, Str::snake($name)],
            File::get(__DIR__.'/stubs/tag-method.stub')
        );
    }
}
